...
export default function ToolsHomeClient({ initialUnlocked = false }: { initialUnlocked?: boolean }) {
  ...
  if (!isUnlocked) {
    return (
      <main className="grid min-h-dvh place-items-center bg-[#f3f6fb] px-6 text-[#172033]">
        <section className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleUnlock} className="mt-6 space-y-3">
            <label className="block text-sm font-black text-slate-700" htmlFor="tools-password">
              パスワード
            </label>
            <input
              id="tools-password"
              ...
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold text-slate-900 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100"
            />
            {error ? <p className="text-xs font-bold text-red-600">{error}</p> : null}
            <button
              type="submit"
              className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-gradient-to-r from-purple-700 via-indigo-600 to-cyan-500 px-5 text-sm font-black text-white shadow-md shadow-indigo-900/20 transition hover:brightness-105"
            >
              一覧を表示
            </button>
          </form>
