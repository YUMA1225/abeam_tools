...
const categoryIcon: Record<SecurityCategoryKey, ReactNode> = {
  ssl: <ShieldIcon />,
...
export default function Home() {
  ...
  const [activeKey, setActiveKey] = useState<SecurityCategoryKey>("ssl");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = (await response.json()) as SecurityAnalyzeResponse;
      if (!payload.ok) {
...
function Report({
  ...
  activeKey: SecurityCategoryKey;
  onTabChange: (key: SecurityCategoryKey) => void;
  onCopy: () => void;
  copied: boolean;
}) {
...
function countStatuses(checks: SecurityCheck[]) {
...
function exportReportPdf(report: SecurityReport) {
...
function buildReportHtml(report: SecurityReport) {
  const sorted = report.categories.slice().sort((a, b) => categoryOrder.indexOf(a.key) - categoryOrder.indexOf(b.key));
  const totalChecks = sorted.reduce((sum, category) => sum + category.checks.length, 0);
...
<head>
  <meta charset="utf-8" />
  <title>セキュリティ診断レポート - ${escapeHtml(report.finalUrl)}</title>
  <style>
...
<body>
  <main class="page">
    <header>
      <div class="report-kicker">SECURITY AUDIT REPORT</div>
      <h1>セキュリティ診断レポート</h1>
      <div class="meta">
    ...
    ${categoryDetails}
    <div class="footer">Generated by セキュリティ診断ツール (by Abeam Tech)</div>
  </main>
</body>
</html>`;
}
...
function shouldShowPdfValue(check: SecurityCheck) {
